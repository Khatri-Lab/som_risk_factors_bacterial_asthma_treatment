...
###############################################################
plot_cor_risk_factor <- function(score_pheno_frame, group_name)
{
  group_name_plot <- "Framingham (all)"
  if(group_name != "All")
  {
    score_pheno_frame <- score_pheno_frame[score_pheno_frame$risk_factors_grouped == group_name, ]
    group_name_plot <- paste0(group_name, " risk factors")
  }
  ...
  corr_summary <- ggplot(score_pheno_frame, aes(x = detrimental_score_scaled, y = protective_score_scaled)) +
    ...
    geom_point(size = point_size, colour = black_line_colour) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
    stat_smooth(method = "lm", formula = y ~ x, geom = "smooth", lwd = line_size_main_mm) +
    stat_cor(p.accuracy = 0.01, r.accuracy = 0.01, size = text_size_labels, label.y = Inf, label.x = 3, hjust = 1, vjust = 1.5) +
    theme(legend.position = "none")
  return(corr_summary)
}
cor_0_risk <- plot_cor_risk_factor(coconut_combined_risk_factors, "0")
cor_1_3_risk <- plot_cor_risk_factor(coconut_combined_risk_factors, "1 - 3")
cor_4_risk <- plot_cor_risk_factor(coconut_combined_risk_factors, "\u2265 4")
cor_all_risk <- plot_cor_risk_factor(coconut_combined_risk_factors, "All")
cor_by_risk <- plot_grid(cor_0_risk, cor_1_3_risk, cor_4_risk, nrow = 1, align = "hv", axis = "tblr")
###############################################################


###############################################################
plot_age_sex_interaction <- function(score_pheno_frame, score_name, score_name_plot)
{
  score_pheno_frame$score_name_scaled <- scale(score_pheno_frame[[score_name]])
  age_sex_interaction <- ggplot(score_pheno_frame, aes(x = age, y = score_name_scaled, color = sex)) +
    xlab("Age") + ylab(score_name_plot) +
    geom_point(size = 0.5 * point_size, alpha = 0.1) +
    stat_smooth(method = "loess", formula = y ~ x, geom = "smooth", lwd = 1.5 * line_size_main_mm, show.legend = FALSE) +
    theme(legend.position = "bottom", legend.direction = "horizontal", legend.margin = margin(1, 1, 1, 1), legend.box.margin = margin(-8, 5, 3, 3)) +
    scale_color_manual(values = c(`F` = "#AA3377", `M` = "#4477AA"), labels = c(`F` = "Female", `M` = "Male")) +
    labs(color = NULL) + guides(color = guide_legend(nrow = 1, override.aes = list(size = text_size_labels * 6 / 9, alpha = 1)), fill = "none", shape = "none")
  return(age_sex_interaction)
}
som_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "som_score", "SoM")
detrimental_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "detrimental_score", "Detrimental") + theme(legend.position = "none")
protective_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "protective_score", "Protective") + theme(legend.position = "none")
module_1_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "module_1_score", "Module 1") + theme(legend.position = "none")
module_2_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "module_2_score", "Module 2") + theme(legend.position = "none")
module_3_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "module_3_score", "Module 3") + theme(legend.position = "none")
module_4_age_sex_interaction <- plot_age_sex_interaction(coconut_combined[coconut_combined$healthy == 1, ], "module_4_score", "Module 4") + theme(legend.position = "none")
age_sex_interaction_legend <- as_ggplot(ggpubr::get_legend(som_age_sex_interaction))
age_sex_interaction_top <- plot_grid(som_age_sex_interaction + theme(legend.position = "none"), detrimental_age_sex_interaction, protective_age_sex_interaction, nrow = 1, align = "hv", axis = "tblr")
age_sex_interaction_bottom <- plot_grid(module_1_age_sex_interaction, module_2_age_sex_interaction, module_3_age_sex_interaction, module_4_age_sex_interaction, nrow = 1, align = "hv", axis = "tblr")
###############################################################
